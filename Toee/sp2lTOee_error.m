function sp2lTOee_error(P,Def_Base,Results)
...
elseif ( strcmp(P,'Single') )
    ...
    for ivar=1:length(Def_Base.Var_out)
        NET =  Results.(Def_Base.Algorithm_Name).(Def_Base.Name).(Def_Base.Var_out{ivar}).NET;
        netdata = [0 0 ivar 0 1];
        xmin = NET.inputs{1,1}.processSettings{1,1}.xmin;
        xmax = NET.inputs{1,1}.processSettings{1,1}.xmax;
        ymin = NET.inputs{1,1}.processSettings{1,1}.ymin;
    ...
    export(mat2dataset(tabledata),'File',[Def_Base.Name '_' Def_Base.Algorithm_Name '_' P '_0_1_Error.csv'],'Delimiter',',')
